...
RUN mkdir data
CMD /usr/local/bin/casserole server --address=0.0.0.0:80 --mirror-url=${UPSTREAM_SERVER} --peering-address=http://${HOSTNAME}:8080 --etcd=${ETCD} --passthrough='\.pom$,\.pom\.sha1$,\.xml$,\.xml\.sha1$'
