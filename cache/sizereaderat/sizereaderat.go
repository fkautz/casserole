...
//}
//
//type chunkAlignedReader struct {
//	reader    SizeReaderAt
//	chunkSize int
//	size      int64
//}
//
//func (reader chunkAlignedReader) ReadAt(p []byte, offset int64) (int, error) {
//	i64ChunkSize := int64(reader.chunkSize)
//	startChunk := offset / i64ChunkSize
//	endChunk := (offset + int64(len(p))) / i64ChunkSize
//	if totalUpstreamChunks := reader.size / i64ChunkSize; totalUpstreamChunks < endChunk {
//		endChunk = reader.size / i64ChunkSize
//	}
//
//	result := bytes.NewBuffer(p)
//
//	totalRead := 0
//	buf := make([]byte, reader.chunkSize, reader.chunkSize)
//	for curChunk := startChunk; curChunk <= endChunk; curChunk++ {
//		curRead, err := reader.ReadAt(buf, curChunk*i64ChunkSize)
//		chunkOffset := 0
//		if curChunk == startChunk {
//			// TODO test unsafe cast
//			chunkOffset = int(offset % i64ChunkSize)
//		}
//		result.Write(p[chunkOffset : chunkOffset+curRead])
//		totalRead += curRead
//		if err != nil {
...
//}
//
//func (reader chunkAlignedReader) Size() int64 {
//	return reader.size
//}
